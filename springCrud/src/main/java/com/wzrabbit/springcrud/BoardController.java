...
public class BoardController {
	...
	@RequestMapping(value = "/write", method = RequestMethod.GET) // GET 방식으로 페이지 호출
	...
	@RequestMapping(value = "/write", method = RequestMethod.POST) // POST방식으로 내용 전송
	  ...
	  public String registPOST(BoardVO board, RedirectAttributes rttr) throws Exception { // 인자값으로 REDIRECT 사용
		...
		service.write(board); // 글작성 서비스 호출
	...
	@RequestMapping(value = "/read", method = RequestMethod.GET) // GET 방식으로 페이지 호출
	  ...
	  public void read(@RequestParam("id")Integer id, Model model) throws Exception {
		
		 model.addAttribute(service.read(id)); // read 서비스 호출

	  }
	
	@RequestMapping(value = "/update", method = RequestMethod.GET) // GET 방식으로 페이지 호출

	  public void modifyGET(Integer id, Model model) throws Exception {

	    model.addAttribute(service.read(id)); // 수정을 위한 글읽기 서비스 호출

	  }

	  @RequestMapping(value = "/update", method = RequestMethod.POST)// POST방식으로 데이터 전송

	  public String modifyPOST(BoardVO board, RedirectAttributes rttr) throws Exception {

	    service.update(board); // 글수정 서비스 호출
	  ...
	  public String removePOST(@RequestParam("id") Integer id, RedirectAttributes rttr) throws Exception{
